...
const sections = [
  {
    label: "Overview",
    items: [
      { path: "/registrar/dashboard", label: "Dashboard", icon: LayoutDashboard },
    ],
  },
  {
    label: "Student Management",
    items: [
      { path: "/registrar/new-student", label: "New Student", icon: UserPlus },
      { path: "/registrar/students", label: "Student Records", icon: Users },
      { path: "/registrar/stats", label: "Enrollment Stats", icon: BarChart3 },
    ],
  },
  {
    label: "Events",
    items: [
      { path: "/registrar/events", label: "Events", icon: Calendar },
    ],
  },
];

export default function RegistrarSidebar({ collapsed, onToggleCollapse, ...props }) {
            ...
            {!collapsed && (
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-muted-foreground truncate">
                  {user?.email || "registrar@university.edu"}
                </p>
              </div>
            )}
          </div>
