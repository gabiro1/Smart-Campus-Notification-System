...
export default function HrTopbar({ title = "Dashboard", onMenuClick, onCollapseClick, collapsed }) {
  ...
  return (
    <header className="h-14 bg-card border-b border-border flex items-center justify-between px-4 lg:px-6 gap-4 shrink-0 relative">
      ...
      <div className="hidden md:flex items-center gap-2 bg-background border border-border rounded-lg px-3 py-2 text-muted-foreground text-[13px] cursor-text hover:border-violet-500/50 transition-colors flex-1 max-w-sm relative">
          ...
          {loading ? (
            ...
            <div className="py-2">
              ...
              {searchResults.users.map((usr) => (
                <button
                  key={usr._id}
                  onClick={() => handleResultClick("user", usr._id)}
                  className="w-full flex items-center gap-3 px-3 py-2 hover:bg-accent text-left"
                >
                  <Users size={14} className="text-purple-400 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-foreground truncate">{usr.name}</p>
      ...
      <div className="flex items-center gap-2">
        <div className="hidden sm:flex items-center gap-2 bg-violet-500/10 border border-violet-500/20 text-violet-500 px-3 py-1.5 rounded-full text-xs font-medium">
          <ShieldCheck size={14} /> HR Access
        </div>
        ...
        <div className="flex items-center gap-3 pl-3 border-l border-border">
          <div className="text-right hidden sm:block">
            <p className="text-sm font-semibold text-foreground">{user?.name || "HR Officer"}</p>
            <p className="text-xs text-muted-foreground">Human Resources</p>
          </div>
          <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-violet-600 to-purple-600 p-0.5">
