...
const quickStatCards = [
  { label: "Upcoming Events", icon: Calendar, color: "text-blue-500", bg: "bg-blue-500/10", link: "/student/events", key: "events" },
  { label: "Announcements", icon: Megaphone, color: "text-purple-500", bg: "bg-purple-500/10", link: "/student/announcements", key: "announcements" },
  { label: "Bookmarks", icon: Bookmark, color: "text-amber-500", bg: "bg-amber-500/10", link: "/student/bookmarks", key: "savedCount" },
  { label: "Active Now", icon: TrendingUp, color: "text-emerald-500", bg: "bg-emerald-500/10", link: "/student/events", key: "campusPulse" },
];
...
export default function StudentDashboard() {
    ...
    const fetchDashboard = async () => {
      try {
        ...
        if (summaryRes.status === "fulfilled" && summaryRes.value?.success) {
          ...
          setStats({
            attendanceRate: d.stats?.attendanceRate ?? 0,
            aiMatchAvg: d.stats?.aiMatchAvg ?? 0,
            savedCount: d.stats?.savedCount ?? 0,
            campusPulse: d.stats?.campusPulse ?? 0,
          });
        ...
        if (statsRes.status === "fulfilled" && statsRes.value?.success) {
          ...
          if (s) {
            setStats((prev) => ({
              ...prev,
              events: s.events ?? prev?.events ?? 0,
              announcements: s.announcements ?? prev?.announcements ?? 0,
              savedCount: s.bookmarks ?? prev?.savedCount ?? 0,
              campusPulse: s.messages ?? prev?.campusPulse ?? 0,
            }));
          }
        }
  ...
  const s = stats || { events: 3, announcements: 5, savedCount: 2, campusPulse: 12 };
  const displayStats = quickStatCards.map((card) => ({
    ...card,
    value: s[card.key] ?? "—",
  }));
        ...
        {displayStats.map((stat, i) => (
          <Link key={i} to={stat.link}>
            <GlassCard delay={i * 0.08} hoverOffset={-3} className="flex flex-col gap-3 cursor-pointer">
              ...
              <div>
                <h3 className="text-3xl font-bold text-foreground tracking-tight">{loading ? "..." : stat.value}</h3>
