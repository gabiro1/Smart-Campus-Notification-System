...
export default function StudentDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white flex font-sans overflow-x-hidden">
      ...
      <main className="flex-1 ml-20 md:ml-72 p-4 sm:p-6 md:p-8 min-h-screen overflow-y-auto transition-all duration-300">
        {/* Centering Wrapper */}
        <div className="max-w-7xl mx-auto">
          <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 md:mb-10">
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              <div className="bg-white/5 px-4 py-2 rounded-[7px] border border-white/10 flex items-center gap-2 w-full sm:w-auto justify-center">
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center mb-8 gap-3 md:gap-4">
            <div className="relative flex-1 w-full md:max-w-md">
              ...
              <input
                ...
                className="w-full bg-[#111111] border border-white/10 rounded-[7px] py-2.5 md:py-3 pl-11 pr-4 text-sm focus:outline-none focus:border-blue-500/50 transition-all placeholder:text-neutral-600 text-white"
              />
            </div>
            <button
              onClick={() =>
              ...
              className={`border px-4 py-2.5 md:py-3 rounded-[7px] text-xs font-bold flex items-center justify-center gap-2 transition-colors w-full md:w-auto ${
                eventFilter === "top"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 md:gap-4 pb-10">
            <div className="lg:col-span-2 bg-[#0D0D0D] border border-white/5 rounded-[10px] p-4 sm:p-6 md:p-8">
              <div className="flex justify-between items-start mb-6 md:mb-8">
                ...
                <span className="text-[9px] bg-blue-600/20 text-blue-500 px-2 py-1 rounded font-black tracking-tighter uppercase shrink-0">
                  AI Active
                </span>
              </div>
                  ...
                  displayEvents.map((event) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={`p-5 md:p-6 rounded-[15px] border transition-all relative group flex flex-col justify-between ${
                          isHighMatch
                      ...
                      >
                        <div>
                          <div className="flex justify-between items-start mb-4">
                            <span
                              className={`text-[9px] md:text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded ${
                                isHighMatch
                              ...
                              {[1, 2, 3, 4, 5].map((star) => {
                                ...
                                return (
                                  <Star
                                    key={star}
                                    size={12}
                                    className={`cursor-pointer transition-colors p-0.5 md:p-0 ${
                                      currentRating >= star
                          ...
                          <h4
                            className={`font-bold text-white mb-2 leading-tight ${isHighMatch ? "text-lg md:text-xl" : "text-sm md:text-md"}`}
                          >
                            {event.title}
                          </h4>
                          <p className="text-[11px] md:text-xs text-neutral-400 line-clamp-2 mb-5 md:mb-6 leading-relaxed">
                            {event.description}
                          </p>
                        </div>
                        ...
                        <div className="flex justify-between items-center mt-auto border-t border-white/5 pt-4">
                          <button
                            ...
                            className={`text-[9px] md:text-[10px] font-bold flex items-center gap-1.5 transition-all duration-200 ${
                              event.interested
                                ? "text-red-400"
                          ...
                          <button className="bg-white/5 hover:bg-white/10 p-1.5 md:p-2 rounded-lg transition-colors text-neutral-400 hover:text-white">
                            <ArrowRight size={14} />
                          </button>
                        </div>
                      </motion.div>
            ...
            <div className="flex flex-col gap-3 md:gap-4">
              <motion.div
                ...
                className="bg-[#0D0D0D] border border-white/5 rounded-[10px] p-5 md:p-7"
              >
                  ...
                  {[
                    ...
                    >
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-[11px] md:text-xs font-bold text-neutral-200">
                          {msg.sender}
                        </span>
                      ...
                      <p className="text-[10px] md:text-[11px] text-neutral-400 line-clamp-1 italic">
                        "{msg.text}"
                      </p>
                    </div>
                  ))}
                </div>
                <button className="w-full mt-4 text-[9px] md:text-[10px] font-bold text-neutral-600 hover:text-white transition-colors">
                  Open All Messages
                </button>
              </motion.div>

              <div className="bg-[#0D0D0D] border border-white/5 rounded-[10px] p-5 md:p-7 flex-1">
                <div className="flex items-center justify-between mb-5 md:mb-6">
                  <h3 className="text-sm font-bold flex items-center gap-2">
                    <Clock size={16} className="text-emerald-500" /> Today's
                    Class
                  </h3>
                </div>
                <div className="space-y-3">
                  {[
                    ...
                    <div
                      key={i}
                      className="flex gap-3 md:gap-4 items-center p-3 md:p-4 bg-[#141414] rounded-[10px] border border-white/5"
                    >
                      <div className="text-[10px] font-black text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded shrink-0">
                        {item.time}
                      </div>
                      <div className="flex flex-col min-w-0">
                        <span className="text-[11px] md:text-xs font-bold text-neutral-200 truncate">
                          {item.subject}
                        </span>
                        <span className="text-[9px] md:text-[10px] text-neutral-500 uppercase font-bold mt-0.5">
                          {item.room}
                        </span>
                      </div>
                    </div>
...
function StatCard({ title, value, change, color, icon }) {
  return (
    <motion.div
      ...
      className="bg-[#0D0D0D] p-5 md:p-6 rounded-[10px] border border-white/5 transition-all shadow-sm flex flex-col justify-between"
    >
      <div className="flex justify-between items-start mb-3 md:mb-4">
        <p className="text-white font-black text-[9px] md:text-[10px] uppercase tracking-widest opacity-60">
          {title}
        </p>
        {icon && <div className={color}>{icon}</div>}
      </div>
      <h2
        className={`text-3xl md:text-4xl font-bold tracking-tighter ${color} mb-2 md:mb-3`}
      >
        {value}
      </h2>
      <div className="bg-white/5 text-[8px] md:text-[9px] font-bold px-2 py-1 rounded text-neutral-400 border border-white/5 w-fit uppercase tracking-wide">
        {change}
      </div>
    </motion.div>
  );
