  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
      ...
      <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-red-600 hover:bg-red-500 text-white rounded-xl text-sm font-semibold transition-all shadow-lg shadow-red-600/20 hover:shadow-red-600/40"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((stat, index) => (
          <motion.div
            ...
            className="bg-gradient-to-br from-[#0F1117] to-[#161B22] border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-all group"
          >
            ...
            <p className="text-2xl font-bold text-white">{isLoading ? <Loader2 className="animate-spin" size={20} /> : stat.value}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-[#0F1117] border border-white/5 rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-white/5">
            {isLoading ? (
              ...
              <div className="py-16 flex flex-col items-center justify-center text-center px-6">
                ...
                <p className="text-neutral-400 font-medium">No announcements yet</p>
                <button 
              ...
              recentAnnouncements.map((ann, index) => (
                ...
                >
                  <div className="flex items-start justify-between gap-4 mb-2">
                    <h3 className="text-sm font-semibold text-white line-clamp-1">{ann.title}</h3>
                  ...
                  <p className="text-xs text-neutral-500 line-clamp-2 mb-3">{ann.content}</p>
          ...
          <div className="bg-[#0F1117] border border-white/5 rounded-2xl p-5">
            ...
            <div className="space-y-2">
              ...
              <button 
                ...
                className="w-full flex items-center gap-3 p-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-neutral-300 text-sm font-medium transition-all group"
              >
              ...
              {classes.slice(0, 3).map((cls) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-white">{cls.code}</p>
                      <p className="text-xs text-neutral-500 truncate max-w-[120px]">{cls.name}</p>
                    </div>
                  </div>
                  <span className="text-xs text-neutral-500">{cls.studentCount || 0}</span>
                </div>
              ))}
              {classes.length === 0 && (
                <p className="text-center text-sm text-neutral-500 py-4">No classes assigned</p>
              )}
            </div>
          </div>
        </div>
