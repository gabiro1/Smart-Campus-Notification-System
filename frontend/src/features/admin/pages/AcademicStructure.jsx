...
export default function AcademicStructure() {
  ...
  const generateCode = (name, tab = activeTab) => {
    ...
    // For courses, format: [DeptCode]E[Year][3-digit] e.g., ITE4162
    if (tab === "courses" && formData.class) {
      const selectedClass = classes.find(c => c._id === formData.class);
      const deptCode = selectedClass?.department?.code || baseCode;
      const level = (selectedClass?.level || "Year 1").replace(/\D/g, '');
      const classCourses = courses.filter(c => String(c.class?._id) === String(formData.class) || String(c.class) === String(formData.class));
      const courseNum = (classCourses.length + 1).toString().padStart(3, '0');
      return `${deptCode}E${level}${courseNum}`;
    }
  ...
  // Append year level to class code when level changes
  const handleLevelChange = (level) => {
    setFormData((prev) => {
      const yearNum = level.replace(/\D/g, '');
      const baseCode = prev.code.replace(/\d+$/, '');
      const isAutoGenerated = prev.code === generateCode(prev.name, "classes");
      return {
        ...prev,
        level,
        code: isAutoGenerated || baseCode !== prev.code ? (baseCode + yearNum).toUpperCase() : prev.code,
      };
    });
  };
    ...
    switch (tab) {
      ...
      case "classes":
        return {
          ...
          level: item?.level || "",
          academicYear: item?.academicYear || `${new Date().getFullYear()}-${new Date().getFullYear() + 1}`,
        };
      case "courses":
        return {
          name: item?.name || "",
...
// Render lecturer assignment to courses/classes
      ...
      case "courses":
        return courses.map((course) => (
          <ListItem
            ...
            badge={`${course.semester || "Semester 1"}`}
            extra={`${course.class?.name || ""} • ${course.lecturer?.name || "No Lecturer"}`}
  ...
  return (
    <div ref={containerRef} className="p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6 w-full text-foreground">
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          {activeTab !== "students" && activeTab !== "lecturers" && (
            <button
              onClick={() => {
                const entityMap = { colleges: "colleges", schools: "schools", departments: "departments", classes: "classes", courses: "courses" };
                setBulkUploadEntity(entityMap[activeTab]);
                setShowBulkUpload(true);
              }}
              className="group relative flex-1 sm:flex-initial px-3 sm:px-5 py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-300 flex items-center justify-center gap-1.5 sm:gap-2.5 overflow-hidden border border-blue-500/20 hover:border-blue-500/40 bg-gradient-to-r from-blue-500/10 to-purple-500/10 hover:from-blue-500/20 hover:to-purple-500/20 text-blue-300 hover:text-blue-200 shadow-[0_0_12px_rgba(59,130,246,0.08)] hover:shadow-[0_0_20px_rgba(59,130,246,0.15)]"
            >
              <Upload size={15} className="shrink-0 transition-transform duration-300 group-hover:scale-110 group-hover:-translate-y-0.5" />
          ...
          {activeTab !== "students" && (
            <button
              ...
              className="flex-1 sm:flex-initial bg-blue-600 hover:bg-blue-500 text-white px-3 sm:px-5 py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-all shadow-[0_0_15px_rgba(59,130,246,0.3)] flex items-center justify-center gap-1.5 sm:gap-2"
            >
              <Plus size={16} className="shrink-0" />
              <span className="hidden xs:inline">Add {activeTab === "lecturers" ? "Lecturer" : activeTab.slice(0, -1)}</span>
      ...
      {/* Tabs */}
      <div className="flex gap-1.5 sm:gap-2 pb-1 sm:pb-2 overflow-x-auto [-webkit-overflow-scrolling:touch] [&::-webkit-scrollbar]:hidden">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-[11px] sm:text-sm font-medium transition-all whitespace-nowrap shrink-0 ${
              activeTab === tab.id
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div style={{ left: sidebarOffset, width: `calc(100% - ${sidebarOffset}px)` }} className="fixed top-0 right-0 bottom-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-md">
            <motion.div
              initial={{ opacity: 0, y: 50 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 50 }}
              className="bg-card rounded-t-2xl sm:rounded-2xl border border-border w-full sm:max-w-lg shadow-2xl max-h-[90vh] sm:max-h-[85vh] overflow-y-auto [&::-webkit-scrollbar]:hidden [scrollbar-width:none]"
            >
              <div className="sticky top-0 bg-card z-10 flex items-center justify-between p-4 sm:p-6 border-b border-border">
                <div className="flex items-center gap-2 min-w-0">
                  <h2 className="text-base sm:text-xl font-bold text-foreground truncate">
                    {editingItem ? "Edit" : "Add"} {activeTab.slice(0, -1)}
                  </h2>
                </div>
                <button onClick={() => setIsModalOpen(false)} className="p-2 rounded-xl hover:bg-accent text-muted-foreground hover:text-foreground shrink-0">
                  <X size={20} />
                </button>
              </div>

              <form className="p-4 sm:p-6 space-y-3 sm:space-y-4" onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
                {activeTab === "colleges" && (
                  <>
                ...
                {activeTab === "classes" && (
                  <>
                    ...
                    <FormSelect label="Level/Year" value={formData.level} onChange={handleLevelChange} options={[
                ...
                {activeTab === "courses" && (
                  <>
                    ...
                    <p className="text-xs text-muted-foreground">Course code format: [DEPT]E[YEAR][3-DIGIT] (e.g., ITE4162 = IT Dept Year 4 Course 162)</p>
                    <FormSelect label="Semester" value={formData.semester} onChange={(v) => setFormData({ ...formData, semester: v })} options={[
                ...
                <div className="sticky bottom-0 bg-card border-t border-border mt-3 sm:mt-4 -mx-4 sm:-mx-6 px-4 sm:px-6 py-3 sm:py-4 flex justify-end gap-2 sm:gap-3">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="flex-1 sm:flex-none px-4 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent transition-all"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-500 text-white px-5 sm:px-6 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                  >
        ...
        {isUserModalOpen && (
          <div style={{ left: sidebarOffset, width: `calc(100% - ${sidebarOffset}px)` }} className="fixed top-0 right-0 bottom-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-md">
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 50 }}
              className="bg-card rounded-t-2xl sm:rounded-2xl border border-border w-full sm:max-w-md shadow-2xl max-h-[90vh] sm:max-h-[85vh] overflow-y-auto [&::-webkit-scrollbar]:hidden [scrollbar-width:none]"
            >
              <div className="sticky top-0 bg-card z-10 flex items-center justify-between p-4 sm:p-6 border-b border-border">
                <h2 className="text-base sm:text-xl font-bold text-foreground truncate pr-2">
                  Create New {userRoleToAssign === 'principal' ? 'Principal' : userRoleToAssign === 'dean' ? 'Dean' : userRoleToAssign === 'lecturer' ? 'Lecturer' : 'HoD'}
                </h2>
                <button onClick={() => setIsUserModalOpen(false)} className="p-2 rounded-xl hover:bg-accent text-muted-foreground hover:text-foreground shrink-0">
                  <X size={20} />
                </button>
              </div>

              <form className="p-4 sm:p-6 space-y-3 sm:space-y-4" onSubmit={(e) => { e.preventDefault(); handleCreateUserAndAssign(); }}>
                ...
                <div className="sticky bottom-0 bg-card border-t border-border mt-3 sm:mt-4 -mx-4 sm:-mx-6 px-4 sm:px-6 py-3 sm:py-4 flex justify-end gap-2 sm:gap-3">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-1 sm:flex-none bg-green-600 hover:bg-green-500 text-white px-5 sm:px-6 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                    {loading ? <Activity size={18} className="animate-spin" /> : <Save size={18} />}
                    Create & Assign
                  </button>
      ...
      <BulkUploadModal
        ...
        onClose={() => { setShowBulkUpload(false); setBulkUploadEntity(null); }}
        ...
        {isCourseAssignModalOpen && selectedLecturerForAssign && (
          <div style={{ left: sidebarOffset, width: `calc(100% - ${sidebarOffset}px)` }} className="fixed top-0 right-0 bottom-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-md">
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 50 }}
              className="bg-card rounded-t-2xl sm:rounded-2xl border border-border w-full sm:max-w-md shadow-2xl max-h-[85vh] overflow-y-auto [&::-webkit-scrollbar]:hidden [scrollbar-width:none]"
            >
              <div className="sticky top-0 bg-card z-10 flex items-center justify-between p-4 sm:p-6 border-b border-border">
                <h2 className="text-base sm:text-xl font-bold text-foreground truncate pr-2">
                  Assign Course
                </h2>
                <button onClick={() => setIsCourseAssignModalOpen(false)} className="p-2 rounded-xl hover:bg-accent text-muted-foreground hover:text-foreground shrink-0">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-4 sm:p-6 space-y-3 sm:space-y-4">
                ...
                <div className="sticky bottom-0 bg-card border-t border-border -mx-4 sm:-mx-6 px-4 sm:px-6 py-3 sm:py-4">
                  <button
                    type="button"
                    onClick={() => setIsCourseAssignModalOpen(false)}
                    className="w-full sm:w-auto px-5 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent transition-all"
                  >
                    Close
                  </button>
                </div>
  ...
  return (
    <div className="space-y-1">
      ...
      <input
        ...
        className="w-full bg-card border border-border p-3 sm:p-4 rounded-xl focus:border-blue-500 outline-none text-sm text-foreground"
      />
    </div>
  );
}
  ...
  return (
    <div className="space-y-1">
      ...
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="w-full bg-card border border-border p-3 sm:p-4 rounded-xl focus:border-blue-500 outline-none text-sm text-foreground appearance-none cursor-pointer"
      >
        {children}
        {options?.map((opt) => (
    ...
    <div className="space-y-1">
      ...
      <div className="flex gap-2">
        <select
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="flex-1 min-w-0 bg-card border border-border p-3 sm:p-4 rounded-xl focus:border-blue-500 outline-none text-sm text-foreground appearance-none cursor-pointer"
        >
        ...
        <button
          type="button"
          onClick={onCreateNew}
          className="shrink-0 px-3 sm:px-4 py-3 bg-green-600 hover:bg-green-500 text-white rounded-xl text-sm font-medium flex items-center gap-1.5 sm:gap-2 transition-all"
        >
          <Plus size={15} />
          <span className="hidden sm:inline">New</span>
        </button>
      </div>
      <p className="text-[11px] sm:text-xs text-muted-foreground">Select an existing user OR create a new one</p>
    </div>
  );
}
