    ...
    const fetchData = async () => {
      try {
        ...
        const mappedNotifs = rawNotifs.map(n => ({
          ...
          icon: n.type === "alert" ? AlertCircle : n.type === "event" ? Radio : n.type === "approval" ? CheckCircle2 : Bell,
          color: n.type === "alert" ? "text-amber-400" : n.type === "event" ? "text-blue-400" : n.type === "approval" ? "text-green-400" : "text-purple-400",
          bg: n.type === "alert" ? "bg-amber-400/10" : n.type === "event" ? "bg-blue-400/10" : n.type === "approval" ? "bg-green-400/10" : "bg-purple-400/10",
          border: n.type === "alert" ? "border-amber-400/20" : n.type === "event" ? "border-blue-400/20" : n.type === "approval" ? "border-green-400/20" : "border-purple-400/20",
        }));
        
        setNotifications(mappedNotifs);
        setUnreadCount(mappedNotifs.filter(n => n.unread).length);
      } catch {
      ...
      <div className="flex bg-black/40 p-1 rounded-xl border border-white/10 w-fit mx-4 lg:mx-6">
        {[
          ...
          >
            {activeTab === tab.id && (
              <motion.div
                layoutId="admin-notification-tabs"
            ...
            {tab.showBadge && unreadCount > 0 && (
              <span className={`px-1.5 py-0.5 rounded-md text-[10px] font-bold ${activeTab === tab.id ? "bg-blue-500 text-foreground" : "bg-white/10 text-muted-foreground"}`}>
                {unreadCount}
              </span>
            )}
          </button>
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 pt-3 sm:pt-4 mt-3 sm:mt-4 border-t border-border opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button className="flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded-lg bg-red-500/10 text-red-400 text-xs hover:bg-red-500/20 transition-colors">
            ...
            <GlassCard className="p-4 sm:p-6">
              ...
              <div className="prose prose-invert max-w-none">
                {typeof digest === 'string' ? (
                  <p className="textneutral-300 text-sm sm:text-base whitespace-pre-wrap">{digest}</p>
                ) : (
